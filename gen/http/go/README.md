# Go API client for notgamessdkpublic
...
```go
import notgamessdkpublic "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `notgamessdkpublic.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), notgamessdkpublic.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `notgamessdkpublic.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), notgamessdkpublic.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `notgamessdkpublic.ContextOperationServerIndices` and `notgamessdkpublic.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), notgamessdkpublic.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), notgamessdkpublic.ContextOperationServerVariables, map[string]map[string]string{
...
auth := context.WithValue(
		context.Background(),
		notgamessdkpublic.ContextAPIKeys,
		map[string]notgamessdkpublic.APIKey{
